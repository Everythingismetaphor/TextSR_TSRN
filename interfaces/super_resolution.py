...
class TextSR(base.TextBase):
    ...
    def test(self):
        ...
        for i, data in (enumerate(test_loader)):
            ...
            #images_sr = images_lr   # 作者之前写的，图片不放到模型，应该是用来测试原始数据的识别。运行维度不匹配？ 计算ssim图像大小不一样 lr 16*64 hr 32*128
            sr_end = time.time()
            ...
            for pred, target in zip(pred_str_sr, label_strs):  #  pred_str_sr, label_strs
    ...
    def statistics(self):
        ...
        for i, data in (enumerate(test_loader)):
            ...
            images_sr = images_hr   # 作者之前写的，图片不放到模型，应该是用来测试原始数据的识别。运行维度不匹配？ 计算ssim图像大小不一样 lr 16*64 hr 32*128
            sr_end = time.time()
            sr_time += sr_end - sr_beigin
            #metric_dict['psnr'].append(self.cal_psnr(images_sr, images_hr))
            #metric_dict['ssim'].append(self.cal_ssim(images_sr, images_hr))
            ...
            for pred, target in zip(pred_str_sr, label_strs):  #  pred_str_sr, label_strs
                if str_filt(pred, 'lower') == str_filt(target, 'lower'):
                    n_correct += 1
                #self.test_display(images_lr, images_sr, images_hr, pred_str_lr, pred_str_sr, label_strs, str_filt)
                #visualized=self.test_display(images_lr, images_sr, images_hr, pred_str_lr, pred_str_sr, label_strs, str_filt)
                # print(f'visualized_numbers:%d',visualized)
            sum_images += val_batch_size
            torch.cuda.empty_cache()
            print('Evaluation: [{}][{}/{}]\t'
                  .format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
